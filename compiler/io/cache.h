...
namespace io {
struct ASTNode;   // forward declared, defined in ast/; cache holds shared_ptr<ASTNode>
...
struct ContentHash
{
    ...
    /// Compute a fast, non-cryptographic 256-bit hash.
    /// Uses four interleaved FNV-1a 64-bit streams to fill all 32 bytes, which is sufficient for cache-invalidation purposes.
...
// Cached module entry
/// Everything the compiler knows about one source module after it has been fully processed. 
/// All fields are optional so the cache can be partially populated (e.g. after lexing but before type-checking).
struct CachedModule
{
    // Identity
    ...
    // Validity check; returns true if current hash matches the stored hash, meaning the
    /// cached data is still valid for the current source on disk.
    bool is_valid(const ContentHash& current_hash) const noexcept { return hash == current_hash; }
};
...
public:
    ...
    void put(const std::string& path, CachedModule entry) { entries_[path] = std::move(entry); }
 
    /// Look up an entry and validate its hash against current_hash. Returns nullptr if absent or stale.
    const CachedModule* get(const std::string& path, const ContentHash& current_hash) const
    {
    ...
    const std::unordered_map<std::string, CachedModule>& entries() const { return entries_; }
...
public:
    ...
    std::optional<CachedModule> get(const std::string& path, const ContentHash& current_hash) const
    {
    ...
    bool contains(const std::string& path, const ContentHash& current_hash) const
    {
        std::shared_lock lock(mutex_);
        auto it = entries_.find(path);
    ...
    /// Entries already present are only overwritten if the incoming hash differs (i.e. the thread resolved a fresher version).
    void merge(const ThreadLocalCache& local)
    {
        ...
        {
            auto it = entries_.find(path);
            if (it == entries_.end() || !it->second.is_valid(entry.hash)) entries_[path] = entry;
        }
    }
