...
Parser::Parser(Lexer& lexer) : lexer(lexer), current(lexer.nextToken()), previous(current) {}
...
void Parser::advance() // consume the current token and read the next one
{
    previous = current;
...
void Parser::expect(TokenKind k, const std::string& msg) // ensure the current token is of the expected kind, or throw an error
{
...
{
    Program program;
    while (!check(TokenKind::EOF_TOKEN)) program.addStmt(parseStatement());
    return program;
}
...
Compound Parser::parseCompound()
{
    ...
    while (!check(TokenKind::RBRACE) && !check(TokenKind::EOF_TOKEN))
    {
        ...
        {
            if (!compound.break_idx.has_value()) compound.setBreakIdx(compound.stmts.size());
        }

        // Detect implicit return: a non-statement-starting token followed eventually by RBRACE with no SEMICOLON
        bool is_stmt_start =
        ...
        else { compound.addStmt(parseStatement()); }
    }
...
StmtPtr Parser::parseLet()
{
    ...
    bool is_grad = false;
    if (match(TokenKind::KW_GRAD)) is_grad = true;
    if (!check(TokenKind::IDENTIFIER)) throw error("expected identifier after 'let'");
    ...
    if (ty == TyKind::Tensor && last_tensor_gp.has_value()) k.let_ident.set_tensor_gp(std::move(*last_tensor_gp));
...
StmtPtr Parser::parseFnDecl()
{
    Position p = current.pos;
    bool is_async = false;
    if (match(TokenKind::KW_ASYNC)) { is_async = true; }
    expect(TokenKind::KW_FN, "expected 'fn'");
    if (!check(TokenKind::IDENTIFIER)) throw error("expected function name after 'fn'");
    std::string fn_name = current.value;
    Position name_pos = current.pos;
    advance();
    // optional generic names  #(T, U)
    std::vector<std::string> generic_names;
    if (check(TokenKind::HASH)) generic_names = parseGenericNames();
    ...
    if (ret_tensor_gp.has_value()) fn_ident.set_tensor_gp(std::move(*ret_tensor_gp));
...
StmtPtr Parser::parseReturn()
{
    ...
    if (!check(TokenKind::RBRACE) && !check(TokenKind::SEMICOLON) && !check(TokenKind::EOF_TOKEN)) { k.ret_expr = parseExpr(); }
    consumeOptionalSemicolon();
    return makeStmt(std::move(k), p);
}
...
StmtPtr Parser::parseIf()
{
    ...
    {
        if (check(TokenKind::KW_IF)) { k.else_or_else_if = parseIf(); }
        else
        {
...
StmtPtr Parser::parseFor()
{
    ...
    if (!check(TokenKind::IDENTIFIER)) throw error("expected loop variable after 'for'");
    StmtKind k;
    ...
    if (!check(TokenKind::KW_FROM)) throw error("expected 'in' after loop variable");
    advance();
...
StmtPtr Parser::parseWhile()
{
    ...
    if (!check(TokenKind::LBRACE)) k.while_cond = parseExpr();
...
StmtPtr Parser::parseImport()
{
    ...
    else { throw error("expected string path or identifier after 'import'"); }

    if (match(TokenKind::KW_AS))
    {
        if (!check(TokenKind::IDENTIFIER)) throw error("expected alias name after 'as'");
...
StmtPtr Parser::parseSpawn()
{
    ...
    if (!check(TokenKind::KW_FN)) throw error("expected 'fn' after 'spawn'");
    StmtKind k;
...
StmtPtr Parser::parseStruct()
{
    ...
    if (!check(TokenKind::IDENTIFIER)) throw error("expected struct name after 'struct'");
    ...
    while (!check(TokenKind::RBRACE) && !check(TokenKind::EOF_TOKEN))
    {
        if (!check(TokenKind::IDENTIFIER)) throw error("expected field name in struct");
...
ExprPtr Parser::parsePostfix()
{
    ...
    while (true)
    {
        ...
        else if (check(TokenKind::DOT))
        {
            Position p = current.pos;
            advance();
            if (!check(TokenKind::IDENTIFIER)) throw error("expected field name after '.'");
        ...
        else if (check(TokenKind::DOUBLE_COLON))
        {
            Position p = current.pos;
            advance();
            if (!check(TokenKind::IDENTIFIER)) throw error("expected name after '::'");
            std::string mem = current.value;
            advance();
            ExprKind k;
        ...
        else if (check(TokenKind::CHANNEL_SEND)) { expr = parseChannelSend(std::move(expr)); }
...
ExprPtr Parser::parseStructLit(const std::string& name, Position p)
{
    ...
    while (!check(TokenKind::RBRACE) && !check(TokenKind::EOF_TOKEN))
    {
        if (!check(TokenKind::IDENTIFIER)) throw error("expected field name in struct literal");
        std::string field_name = current.value;
        advance();
        expect(TokenKind::COLON, "expected ':' after field name");
...
ExprPtr Parser::parseIfExpr()
{
    ...
    if (match(TokenKind::KW_ELSE)) k.else_branch = check(TokenKind::KW_IF) ? parseIfExpr() : parseBlockExpr();
    return makeExpr(std::move(k), p);
}
...
ExprPtr Parser::parseFnExpr()
{
    ...
    if (check(TokenKind::HASH)) k.fn_generic_names = parseGenericNames();
    expect(TokenKind::LPAREN, "expected '('");
    for (const Ident& ident : parseParamList())
        k.fn_params.push_back({ ident.name(), ident.ty_kind() });
    expect(TokenKind::RPAREN, "expected ')'");
    k.fn_ret_type = TyKind::Void;
    if (match(TokenKind::ARROW)) k.fn_ret_type = parseType();
...
ExprPtr Parser::parseSpawnExpr()
{
    ...
    if (call->kind.tag != ExprKind::Tag::Call) { throw error("expected function call after 'spawn'"); }
...
ExprPtr Parser::parseTensorLit()
{
    ...
    if (check(TokenKind::HASH)) k.generic_params = parseGenericParams();
...
std::vector<std::string> Parser::parseGenericNames()
{
    ...
    while (!check(TokenKind::RPAREN) && !check(TokenKind::EOF_TOKEN))
    {
        if (!check(TokenKind::IDENTIFIER)) throw error("expected generic type name");
...
std::vector<Ident> Parser::parseParamList()
{
    ...
    while (!check(TokenKind::RPAREN) && !check(TokenKind::EOF_TOKEN))
    {
        if (!check(TokenKind::IDENTIFIER)) throw error("expected parameter name");
        std::string name = current.value;
        Position p = current.pos;
        advance();
        ...
        TyKind ty = parseType();
        std::string utn;
        if (ty == TyKind::Generic || ty == TyKind::UserDef) utn = previous.value;
        ...
        if (ty == TyKind::Tensor && last_tensor_gp.has_value()) param_ident.set_tensor_gp(std::move(*last_tensor_gp));
