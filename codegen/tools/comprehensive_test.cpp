...
void test_tensor_elem_add()
{
    ...
    auto tensor_ty = Type::tensor(Type::f64(), {8, 8});
    ...
    args.push_back(ValuePtr(a));
    args.push_back(ValuePtr(b));
    
    auto* result = fn->entry()->emit<TensorOpInst>("result", tensor_ty, TensorOpCode::ElemAdd, args);
    fn->entry()->emit<ReturnInst>(result);
...
void test_matmul()
{
    ...
    auto matrix_ty = Type::tensor(Type::f64(), {64, 64});
...
void test_fused_matmul_relu()
{
    ...
    auto matrix_ty = Type::tensor(Type::f64(), {32, 32});
...
void test_branching()
{
    ...
    entry->emit<CondBranchInst>(cmp, true_bb, false_bb);
